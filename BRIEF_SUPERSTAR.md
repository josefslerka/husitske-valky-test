...
**Aktualizace 1. 7. 2026:** upřesněn WP1 (canMove je flag → gate, hromadný toggle linie, AI vozy v lure scénářích) po review otázkách Opuse 4.8. Ověřeno: Lipany mají 3 vozy jednoty (VOZOVA_HRADBA_PRASKY), Kutná Hora 8 hráčových vozů.
...
**Spec (ROZHODNUTO):**
1. Nový stav vozu: `unit.formationClosed` (bool, jen `unitClass === 'wagon'` — pozor, vozových TYPŮ je víc, např. `VOZOVA_HRADBA_PRASKY` u Lipan; rozhoduj VŽDY přes `isWagon()`/unitClass, nikdy přes jméno typu). **Default `true`** při vytvoření (zachová dnešní chování defenzivních scénářů, kde linie „prostě funguje"). Volitelné pole ve scénáři: `{ type: 'VOZOVA_HRADBA', col, row, formation: 'open' }` pro vozy, které mají začít rozepnuté (validátor: akceptovat pole, hodnoty open/closed).
   **AI vozy (ROZHODNUTO po review):** default closed platí i pro AI a AI toggle NEDĚLÁ — takže ve scénáři, kde mají vozy AI couvat (WP0 Lipany lure: „vozy couvají!"), MUSÍ mít vozy té strany explicitně `formation: 'open'`, jinak zpráva lže a vozy stojí. Konkrétně: 3× `VOZOVA_HRADBA_PRASKY` u `lipany_1434` → `formation: 'open'`. Validátor: přidej WARNING „AI strana má zavřené vozy ve scénáři s ai_stance retreat/lure".
2. Význam stavů:
   - **Zavřený (closed):** počítá se do linie (`isInWagonLine`), NEMŮŽE se hýbat, střílet může. Sousední spřátelená NE-vozová jednotka dostává **+5 % obrany** („kryt za hradbou") — přidej do agregace obranných modifikátorů v CombatSystem (vzor: `isDefending` bonus ~322, `getWagonFormationBonuses` ~443).
     **Implementace zákazu pohybu (OPRAVA po review):** žádný „pohyb 0" — `Unit.canMove()` je flag-check (`return !this.hasMoved`). Zákaz udělej gate-em: `if (this.isWagon && this.isWagon() && this.formationClosed) return false;` v `canMove()`. `movement` stat NECH na 1. Ověř, že výpočet/zvýraznění pohybového dosahu jde přes `canMove()` (pokud má vlastní cestu, přidej podmínku i tam) a že `canAct()` + útok zavřeného vozu dál fungují (zavřený vůz střílí!).
...
4. Přepnutí = akce: tlačítko `#btn-formation` v `#unit-actions` (vedle btn-defend, stejný vzor zapojení s `{ signal }` v setupEventListeners). Label přepíná: cs „⛓ Sepnout hradbu" / „⛓ Rozevřít hradbu", en „⛓ Chain the wagons" / „⛓ Open the wall" (i18n klíče `game.closeFort` / `game.openFort`). Viditelné jen pro hráčův vůz, který ještě nejednal. Přepnutí nastaví `hasMoved = true` (střílet potom ještě smí — osádka střílí, zatímco vozataji spínají řetězy).
   **Hromadný toggle (ROZHODNUTO po review — řeší klikací peklo u 8 vozů Kutné Hory):** druhé tlačítko `#btn-formation-line`, cs „⛓ Celá linie" / en „⛓ Whole line" (i18n `game.toggleFortLine`): aplikuje toggle na vybraný vůz A všechny s ním SOUVISLE propojené vozy stejné frakce ve STEJNÉM stavu (BFS přes hex-sousednost). Každý dotčený vůz utratí akci (`hasMoved = true`); vozy, které už jednaly, se přeskočí (a zůstanou v původním stavu — BFS je ale prochází dál, souvislost neruší). Dvoutahový rytmus je záměr, ne bug: kolo N „rozpojit řetězy" (vozy ještě střílí), kolo N+1 vyrazit — přesně tak vypadal noční průlom u Kutné Hory.
   **Discoverability:** když je vybraný ZAVŘENÝ vůz, ukaž v unit info panelu hint cs „⛓ Sepnuto řetězy — vůz se nemůže hýbat" / en „⛓ Chained — the wagon cannot move" (i18n `game.wagonChainedHint`), ať hráč chápe, proč nevidí pohybový dosah.
...
**DoD:** Toggle funguje (eval: přepni, ověř `formationClosed`, `canMove()` false/true, linie bonus přes `isInWagonLine` vrací null pro open); „Celá linie" otevře souvislý řetěz jedním klikem (eval: 3 spojené vozy → 1 klik → všechny open + hasMoved; 4. nespojený vůz nedotčen); kryt +5 % měřitelný; řetězy vidět na screenshotu; hint u zavřeného vozu; save→load zachová stav; validátor 0 chyb; oba jazyky.
